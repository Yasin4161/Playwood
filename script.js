...
class PanelPlacementApp {
    ...
    calculateFormPanels(areaWidth, areaHeight) {
        ...
        // Genişlik için panel sayısı (2.5m'den büyükse)
        if (areaWidth > 2.5) {
            const widthPanels = Math.floor(areaWidth / 2.5);
            panels.push({
                direction: "Genişlik",
                length: areaWidth,
                panelCount: widthPanels,
                panelSize: "40x250cm",
                totalArea: (widthPanels * 0.4 * 2.5).toFixed(2)
            });
        }
        
        // Yükseklik için panel sayısı (2.5m'den büyükse)
        if (areaHeight > 2.5) {
            const heightPanels = Math.floor(areaHeight / 2.5);
            panels.push({
                direction: "Yükseklik", 
                length: areaHeight,
                panelCount: heightPanels,
                panelSize: "40x250cm",
                totalArea: (heightPanels * 0.4 * 2.5).toFixed(2)
            });
        }
    ...
    displayResults(results, beamRequirements, formPanels = []) {
        ...
        // Kalıp panel detaylarını ekle
        if (formPanels.length > 0) {
            const formPanelHTML = formPanels.map(panel => `
                <div class="panel-detail-item form-panel">
                    <span class="panel-type">${panel.panelSize} Kalıp (${panel.direction})</span>
                    <span class="panel-count">${panel.panelCount} adet</span>
                </div>
            `).join('');
            detailsHTML += formPanelHTML;
        }
